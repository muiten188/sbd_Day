...
export default class extends PureComponent {
    ...
    return (
      <View key={key} style={styles.itemList}>
        <FlatList
          ...
          scrollEnabled={true}
        />
        <Button transparent style={{ width: 40, position: 'absolute', right: 0, top: 35, justifyContent: 'center', alignItems: 'center' }}
          onPress={() => {
            this.list.scrollToEnd()
          }}>
          <Icon size={20} name="arrow-right"></Icon>
        </Button>
        <Button transparent style={{ width: 40, position: 'absolute', left: 0, top: 35, justifyContent: 'center', alignItems: 'center' }}
          onPress={() => {
  ...
  buildMenuItem(dataItem) {
    ...
    return (
      <TouchableOpacity style={{ padding: 5, borderRadius: 5, marginRight: 10, height: 105, width: 105, backgroundColor: index == this.state.indexSlider ? '#007db7' : '#cecece' }}
